"""Typed contracts for the spawn engine.

These models are the load-bearing part of the system. They define the
permission model and enforce the strict HandoffEnvelope boundary.
...
The core contracts depend on pydantic only. The CrewAI compile paths
(``RoleSpec.to_agent``, ``TaskBrief.to_task``) import crewai lazily — they
work when the ``crewai`` extra is installed and raise a helpful
ImportError otherwise. Everything else (tier policy, envelopes, budgets)
runs framework-free.
"""
...
def _require_crewai() -> Any:
    """Import and return the crewai module, or raise a helpful error."""
    try:
        import crewai  # noqa: PLC0415 - intentional lazy import
    except ImportError as e:  # pragma: no cover - depends on install extras
        raise ImportError(
            "This code path compiles to CrewAI primitives and needs the "
            "crewai extra: pip install 'crewai'"
        ) from e
    return crewai
...
def budget_llm(
    llm: Any, role_name: str, max_tokens: int, run_budget: Any | None = None
) -> Any:
    ...
    if getattr(current_call, "_orgos_spawn_budget_wrapper", False):
        original_call = llm._orgos_spawn_budget_original
    else:
        original_call = current_call
    llm._orgos_spawn_budget_original = original_call

    # Snapshot the instance's cumulative counter at wrap time. All budget math
    # is relative to this baseline, so a reused LLM instance (the natural way
    # to share provider config across roles/runs) doesn't leak prior-run
    # tokens into this run's per-role cap or the shared RunBudget.
    start_total = llm._token_usage.get("total_tokens", 0)
    state = {"last_total": start_total}
    ...
    def _budgeted_call(
        ...
        used_this_run = llm._token_usage.get("total_tokens", 0) - start_total
        if used_this_run > max_tokens:
            from .audit import BudgetExceeded

            raise BudgetExceeded(
                f"Budget exceeded: {used_this_run} real tokens > {max_tokens} "
        ...
        # Mark this call's tokens as seen BEFORE feeding the shared budget —
        # if add() aborts the run, a later run must not re-charge them.
        delta = used - state["last_total"]
        state["last_total"] = used
        if run_budget is not None:
            run_budget.add(delta, role_name)  # may raise BudgetExceeded
        if used - start_total > max_tokens:
            from .audit import BudgetExceeded

            raise BudgetExceeded(
                f"Budget exceeded: {used - start_total} real tokens > "
                f"{max_tokens} cap for role '{role_name}'. Run aborted."
            )
        return result

    _budgeted_call._orgos_spawn_budget_wrapper = True
...
class RoleSpec(BaseModel):
    """A declarative role. Framework-free; compiles to a CrewAI Agent on demand."""
    ...
    # str model id, a crewai LLM instance (when the extra is installed), or None.
    model: Any = None
    ...
    def from_agent_dir(cls, agents_root: Path, agent_name: str) -> "RoleSpec":
        """Build a RoleSpec from layered persona files (soul/brain/habits/...)."""
        from .persona_loader import load_agent
        return load_agent(agents_root, agent_name)

    def _build_llm(self, run_budget: Any | None = None) -> Any:
        LLM = _require_crewai().LLM
        llm: Any = None
    ...
    def to_agent(self, **overrides: Any) -> Any:
        """Compile this RoleSpec into a CrewAI Agent (needs ``crewai``).
...
class TaskBrief(BaseModel):
    ...
    def render_expected(self, role: RoleSpec | None = None) -> str:
        ...
        parts.append(
            "\n\n# Handoff format\n"
            "When done, provide your result as a SINGLE JSON OBJECT (no markdown "
            "headings, no prose around it) with these fields: "
            "role (your role name), status (completed|needs_revision|blocked|failed), "
            "summary (markdown string), artifacts (list of strings), "
