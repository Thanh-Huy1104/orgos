...
human-approval gate. The engine's tier policy (see :mod:`orgos.spawn.contracts`)
decides *which* tools need gating; :func:`orgos.spawn.engine._wire_gates` then
injects the approval callback into each gated tool before the crew runs.

This module is intentionally dependency-light (only CrewAI + pydantic) so the
engine has no import edge into orgos's concrete tools. Concrete tools live in
``orgos/tools/`` and subclass ``GatedToolBase`` (or plain CrewAI ``BaseTool``).
"""
