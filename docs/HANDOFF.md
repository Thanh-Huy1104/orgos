...
- **Version:** 2.0.0 (v1 dispatcher deleted; v2 async runtime is the default)
- **Branch:** `main` (published to `origin/main` as of the most recent
  commit; tag `orgos-v2-async-scrum-team` marks the merge)
- **Tests:** ~200 passing, ~30 s locally (`pytest -q`)
- **Comparison result (measured, DeepSeek Chat, full CRUD goal):**
  scrum ~57% cheaper per goal ($0.22 vs $0.51), delivered fewer stories in a
  12-minute window (3 vs 7). Details in [RESULTS.md](RESULTS.md).
- **Recipient-ready:** README documents install → executor pick → target
  repo bootstrap → comparison. `scripts/run_comparison.sh` is a one-command
  reproducer that writes `/tmp/orgos-comparison.html`.
