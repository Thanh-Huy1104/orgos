...
**Reference docs:**
- **[docs/architecture.html](docs/architecture.html)** — how the two layers (agile team + governance engine, vendored at `orgos/spawn`) fit together, with diagrams
...
- **[docs/spawn-rubric.md](docs/spawn-rubric.md)** — governance-layer deep dive (permission tiers, audit, rubric-graded retry); this layer lives at `orgos/spawn`
...
Requirements: Python ≥ 3.11 and git ≥ 2.5 (needs worktree support). The
governance engine (tiers, budgets, audit trail, `spawn()`) is vendored in
this repo at `orgos/spawn` — no extra install needed.
...
Fallback if you have no CLI installed. Uses the governed CrewAI +
...
tests/                     pytest suite (~475 tests, incl. tests/spawn_engine/
                           exercising the orgos/spawn governance engine)
...
`TIER_POLICY`, `spawn()`) lives at `orgos/spawn` (imported as
`orgos.spawn.governance`) and must not be modified by agents or during
experiments — it has its own test suite (`tests/spawn_engine/`).
Everything else in this repo is fair game.
...
- **Governance layer untouched**: the engine lives at `orgos/spawn` —
  off-limits to agent modifications; agents' write tools are locked to
  their own worktrees, which never include this package.
